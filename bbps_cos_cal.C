...
// Main
void bbps_cos_cal ( int run = 366, int event = -1 ){
  ...
  abc.open(Form("bbpreshower_%d_amptointRatio_SH.txt",run));
  fitData.open(Form("fit_results/bbpreshower_%d_FitResults.txt",run));
  ...
  subCanv[0]->SaveAs(Form("plots/PS_cospeak_%d.pdf[",run));
  for( int canC=0; canC<4; canC++ ) subCanv[canC]->SaveAs(Form("plots/PS_cospeak_%d.pdf",run));
  subCanv[3]->SaveAs(Form("plots/PS_cospeak_%d.pdf]",run));

  cout << "The fit parameters have been written to bbpreshower_" << run << "_FitResults.txt" << "." << endl;
  ...
  // cout << "The histograms for pedestal analysis have been written to outFiles/BBPSped_samps_run" << run << ".root" << "." << endl;
  // cout << "The histograms for cosmic calibration have been written to outFiles/BBPScosCal_samps_run" << run << ".root" << "." << endl;
...
void processEvent( int entry = -1 ){
  ...
  for(r = 0; r < kNrows; r++) {
    for(c = 0; c < kNcols; c++) {
      if( (gPulse[r][c+1]==true&&gPulse[r+2][c+1]==true) ){
